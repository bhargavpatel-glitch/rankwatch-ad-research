import { CanonicalAd, SearchResultItem, SearchResponse, MatchExplanation, FilterOptions } from '../types';
import { adStore } from '../services/adStore';
import { parseSearchQuery, normalizeWord, ParsedQuery } from './queryPipeline';

export class HybridSearchEngine {
  private adList: CanonicalAd[] = [];
  private adDocTokens: Map<string, Set<string>> = new Map();
  private adConceptSets: Map<string, Set<string>> = new Map();
  ...
  public search(queryStr: string, filters: FilterOptions = {}): SearchResponse {
    ...
    let candidates = adStore.filterAds(this.adList, filters);

    // If query is empty, return filtered browse results
    if (!hasQuery) {
      const sorted = adStore.sortAds(candidates, filters.sort);
      const page = filters.page || 1;
      const limit = filters.limit || 40;
      const paged = sorted.slice((page - 1) * limit, page * limit);

      const items: SearchResultItem[] = paged.map(ad => ({
        ad,
        score: 1.0,
        confidence: 'high',
        explanation: {
      ...
      const filterAgg = adStore.getAggregatedFilters();
    ...
    const scoredCandidates: SearchResultItem[] = [];
    ...
    for (const ad of candidates) {
      ...
      for (const fmt of parsedQuery.detectedCreativeTypes) {
        if (adCreativeTypeLower.includes(fmt.toLowerCase())) {
          featureScore += 35.0;
          if (!matchedFields.includes('creative format')) matchedFields.push(`format: ${ad.creativeType}`);
      ...
      for (const p of phrases) {
        if (adTitleLower.includes(p)) {
          ...
          phraseScore += 8.0;
          if (!matchedFields.includes('copy')) matchedFields.push('copy');
          highlightTerms.push(p);
        }
      }
      ...
      for (const term of queryTerms) {
        ...
        for (const topic of adTopicsLower) {
          if (topic.includes(term)) {
            featureScore += 16.0;
            termFoundInAd = true;
            if (!matchedFields.includes('topics')) matchedFields.push(`topic: ${topic}`);
            highlightTerms.push(term);
            break;
          }
        }
        ...
        if (adCreativeTypeLower.includes(term)) {
          featureScore += 18.0;
          termFoundInAd = true;
          if (!matchedFields.includes('creative type')) matchedFields.push(`format: ${ad.creativeType}`);
          highlightTerms.push(term);
        }
      ...
      for (const concept of queryConcepts) {
        if (adConcepts.has(concept)) {
          semanticScore += 20.0;
          if (!matchedFields.includes('concepts')) matchedFields.push(`concept: ${concept}`);
        }
      }
      ...
      for (const exp of expandedTerms) {
        if (adTitleLower.includes(exp) || adSummaryLower.includes(exp) || adConcepts.has(exp)) {
          semanticScore += 8.0;
          if (!matchedFields.includes('semantic match')) matchedFields.push('semantic match');
    ...
    if (!filters.sort || filters.sort === 'relevant') {
      ...
      const sortedAds = adStore.sortAds(scoredCandidates.map(c => c.ad), filters.sort);
  ...
  public getSimilarAds(targetAdId: string, limit: number = 6): CanonicalAd[] {
    const target = adStore.getAdById(targetAdId);
    if (!target) return [];
    ...
    const scored: { ad: CanonicalAd; score: number }[] = [];
